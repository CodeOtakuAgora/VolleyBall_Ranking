...
class Resultats extends Bdd
{
    ...
    {
    	return Bdd::getInstance()->conn->query('SELECT * FROM `volley_resultats` INNER JOIN volley_equipes 
            ON volley_resultats.id = volley_equipes.id');
    }

    // fonction publique (visible et utilisable partout dans le projet) 
    // statique (qui garde la meme signature partout dans le projet)
    // qui retourne toutes les catégories ou l'id est égale à l'id passé en parametre
    public static function getResultats($idResultat)
    {
        return Bdd::getInstance()->conn->query('SELECT * FROM `volley_resultats` WHERE `id` = "' . $idResultat . '"')->fetchObject();
    }

    public static function getFilterResultats() 
    {
    	return Bdd::getInstance()->conn->query('SELECT * FROM `volley_resultats` INNER JOIN volley_equipes 
            ON volley_resultats.id = volley_equipes.id 
			UNION SELECT * FROM `volley_resultats` INNER JOIN volley_equipes 
            ON volley_resultats.id = volley_equipes.id ORDER BY victoires DESC, nb_sets_gagnes DESC');
    }
    ...
    public static function updateResultats($victoires, $defaites, $nb_sets_gagnes, $nb_points_total, $id)
    {
        ...
        return Resultats::getResultats($id);
    }
